...
def test_heat_conduction_rate():
    ...
    assert heat_transfer.heat_conduction_rate(
        k_material, area, temperature_difference, thickness
    ) == pytest.approx(expected_heat_conduction_rate)
...
def test_heat_convection_rate():
    ...
    assert heat_transfer.heat_convection_rate(
        h_coefficient, area, temperature_difference
    ) == pytest.approx(expected_heat_convection_rate)
...
def test_heat_radiation_rate():
    ...
    assert heat_transfer.heat_radiation_rate(
        area, surface_temperature, surroundings_temperature
    ) == pytest.approx(expected_heat_radiation_rate, rel=1e-2)
