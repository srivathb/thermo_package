...
def test_enthalpy_change():
    ...
    assert thermodynamics.enthalpy_of_reaction(
        standard_enthalpies, reactants, products
    ) == pytest.approx(expected_enthalpy_change)
...
def test_work_done_constant_pressure():
    ...
    assert thermodynamics.work_done(
        pressure, volume_change, constant_pressure=True
    ) == pytest.approx(expected_work_done)
...
def test_work_done_constant_volume():
    ...
    assert thermodynamics.work_done(
        pressure, volume_change, constant_volume=True
    ) == pytest.approx(expected_work_done)
